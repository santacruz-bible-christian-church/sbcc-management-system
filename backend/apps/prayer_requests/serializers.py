...
class PrayerRequestSerializer(serializers.ModelSerializer):
    ...
    requester_name_display = serializers.CharField(source="requester_display_name", read_only=True)
...
class PrayerRequestAssignSerializer(serializers.Serializer):
    ...
    assigned_to = serializers.PrimaryKeyRelatedField(
        queryset=User.objects.filter(role__in=["pastor", "elder", "admin"])
    )

    def validate_assigned_to(self, value):
        if value.role not in ["pastor", "elder", "admin"]:
            raise serializers.ValidationError("Can only assign to pastors, elders, or admins")
        return value
...
class PrayerRequestSubmitSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        # Skip requester_name validation if requester is provided in data
        # or if the request is from an authenticated user (requester will be set in view)
        request = self.context.get("request")
        is_authenticated = request and request.user and request.user.is_authenticated

        # If no member requester and not authenticated, name is required (unless anonymous)
        if not data.get("requester") and not is_authenticated:
