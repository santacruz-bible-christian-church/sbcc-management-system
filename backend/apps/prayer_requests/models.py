...
class PrayerRequest(models.Model):
    ...
    is_anonymous = models.BooleanField(
        default=False,
        help_text="Hide requester identity from public view"
    )
    is_private = models.BooleanField(
        default=False,
        help_text="Only visible to assigned pastors/elders"
    )
    is_public = models.BooleanField(
        default=False,
        help_text="Can be shared with congregation for prayer"
    )
    ...
    requester_name = models.CharField(
        max_length=100,
        blank=True,
        help_text="Name for non-member submissions"
    )
    requester_email = models.EmailField(
        blank=True,
        help_text="Email for non-member submissions"
    )
    requester_phone = models.CharField(
        max_length=15,
        blank=True,
        help_text="Phone for non-member submissions"
    )
...
class PrayerRequestFollowUp(models.Model):
    ...
    is_private = models.BooleanField(
        default=True,
        help_text="Private notes only visible to pastors/elders"
    )
    created_by = models.ForeignKey(
        User,
        on_delete=models.SET_NULL,
        null=True,
        related_name="prayer_follow_ups"
    )
