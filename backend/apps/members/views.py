...
class MemberViewSet(viewsets.ModelViewSet):
    ...
    filterset_fields = ['ministry', 'status']  # use status for filtering
    ...
    def archive(self, request, pk=None):
        ...
        if member.status == 'archived':
            return Response({'detail': 'Member already archived.'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def set_status(self, request):
        ...
        if status_value == 'archived':
            updated = qs.update(status=status_value, archived_at=now, is_active=False, updated_at=now)
        else:
